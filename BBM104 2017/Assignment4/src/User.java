...
public class User {
	private int user_id;
	private String password,name,username,school_or_educate,date_of_birth,relationshipstatus;
	...
	public User(int user_id ,String alinan_name,String alinan_username,String alinan_password,String alinan_date_of_birth,String alinan_school_or_educate,String alinan_relationshipstatus){
		setUser_id(user_id);
		setName(alinan_name);
		...
		setRelationshipstatus(alinan_relationshipstatus);
	...
	//buralar doldurulacak ÝÝÝÝÝÝ hata vermesi için yazýldý
 ...
 * */	
	...
	public void addFriend(String[] alinan_lines){
		int donecek_sayi,ilkdongu=0,user_bulamama=0,fazlalik=0;
		if(UserCollection.giren_kisi==1){
			for(User such_user_add_friend:UserCollection.users){
				if(such_user_add_friend.username.equals(alinan_lines[2])){//birinci arkadaþ zaten kullanýcý o yüzden 2. arkadaþ aranmalý
					user_bulamama++;
					break;		
				}
			}
			
			for(String arkadas_liste:(UserCollection.kullanici).collection_of_friends){//burda 2.isim, 1.isim(kullanýcý)listesinde olup olmadýgý kontrol ediliyor
				if(arkadas_liste.equals(alinan_lines[2])&&user_bulamama==1){
					fazlalik++;
					System.out.println("This user is already in your friend list!");
					break;
				}
			}
			if(fazlalik==0){//2. isim olmassa direk eklemeye gidiliyor
				for(donecek_sayi=1;donecek_sayi< alinan_lines.length;donecek_sayi++){

					for(User anlamsiz1 :UserCollection.users){
						if(anlamsiz1.username.equals(alinan_lines[donecek_sayi])&&ilkdongu==0&&user_bulamama==1){
							//System.out.println(anlamsiz1.getCollection_of_friends());
							anlamsiz1.collection_of_friends.add(alinan_lines[donecek_sayi+1]);
							System.out.println(alinan_lines[2]+" has been successfully added to your friend list.");
							//System.out.println(anlamsiz1.getCollection_of_friends());
							//System.out.println(alinan_lines[donecek_sayi+1]);
							//System.out.println("ayraçççççç");
							break;			
						}
					}
					for(User anlamsiz2 :UserCollection.users){
						if(anlamsiz2.username.equals(alinan_lines[donecek_sayi])&&ilkdongu==1&&user_bulamama==1){
							//System.out.println(anlamsiz2.getCollection_of_friends());
							anlamsiz2.collection_of_friends.add(alinan_lines[donecek_sayi-1]);
	...
	public void addPost_TXT(String[] alinan_lines){
		double girecek_longitude= Double.parseDouble(alinan_lines[3]);
		double girecek_latitude=Double.parseDouble(alinan_lines[4]);
		String[] gidecek_tagged_user = alinan_lines[5].split(":");
		...
		UserCollection.kullanici.collection_of_posts.add(new TextPost(alinan_lines[2], girecek_longitude, girecek_latitude, eklenecekler_txt));
		}
		if(UserCollection.giren_kisi==0){
			System.out.println("Error: Please sign in and try again.");
		}		
	...
	public void addPost_IMAGE(String[] alinan_lines){
		...
		String[] array_resolution= alinan_lines[7].split("<x>");
		...
		System.out.println("The post has been successfully added.");
		UserCollection.kullanici.collection_of_posts.add(new ImagePost(alinan_lines[2], girecek_longitude, girecek_latitude, eklenecekler_image, alinan_lines[6], array_resolution_width, array_resolution_height));
		}
		if(UserCollection.giren_kisi==0){
			System.out.println("Error: Please sign in and try again.");	
		}
	...
	public void addPost_VIDEO(String[] alinan_lines){
		...
		int girecek_video_dk= Integer.parseInt(alinan_lines[7]);
		...
		System.out.println("The post has been successfully added.");
		UserCollection.kullanici.collection_of_posts.add(new VideoPost(alinan_lines[2], girecek_longitude, girecek_latitude, eklenecekler_video, alinan_lines[6], girecek_video_dk));
		}
		if(UserCollection.giren_kisi==0){
			System.out.println("Error: Please sign in and try again.");	
		}
	 ...
	 * @param alinan_lines command line
	 * */
	public void blockUser(String[] alinan_lines){//2 kere blocklamayý sor ?// blocklanýnca arkadaþlýk gidiyor mu
		int donecek_sayi,ilkdongu=0,kullanici_bulmama=0;
		
		if(UserCollection.giren_kisi==1){
			for(User bloklama:UserCollection.users){
				
				if(bloklama.username.equals(alinan_lines[2])){
					kullanici_bulmama++;
					for(donecek_sayi=1;donecek_sayi< alinan_lines.length;donecek_sayi++){

						for(User anlamsiz1 :UserCollection.users){
							if(anlamsiz1.getUsername().equals(alinan_lines[donecek_sayi])&&ilkdongu==0){
								//System.out.println(anlamsiz1.getCollection_of_blocked_users());
								anlamsiz1.getCollection_of_blocked_users().add(alinan_lines[donecek_sayi+1]);
								System.out.println(alinan_lines[2]+" has been successfully blocked.");
								//System.out.println(anlamsiz1.getCollection_of_blocked_users());
								//System.out.println(alinan_lines[donecek_sayi+1]);
								//System.out.println("ayraçççççç");
								break;
						...
						for(User anlamsiz2 :UserCollection.users){
							if(anlamsiz2.getUsername().equals(alinan_lines[donecek_sayi])&&ilkdongu==1){
								//System.out.println(anlamsiz2.getCollection_of_blocked_users());
								anlamsiz2.getCollection_of_blocked_users().add(alinan_lines[donecek_sayi-1]);
								//System.out.println(anlamsiz2.getCollection_of_blocked_users());
								//System.out.println(alinan_lines[donecek_sayi-1]);
								break;
							}
	...
	public static void ShowPosts(String[] alinan_lines){
		...
		for(User user_showpost:UserCollection.users){
			if(user_showpost.username.equals(alinan_lines[1])){
				show_post_usersizlik++;
				if(user_showpost.collection_of_posts.size()==0){
				...
				if(user_showpost.collection_of_posts.size()!=0){
					...
					System.out.println("**************");
					DateFormat df2 = new SimpleDateFormat("dd/MM/yyyy");
					for(TextPost cikan_post:UserCollection.kullanici.collection_of_posts){
